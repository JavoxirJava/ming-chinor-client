...
export default function BasketPage() {
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            const productCount = await fetchBasket();
            if (productCount === 0) await fetchOrders();
        } catch {
    ...
    const makeOrder = async () => {
        try {
            if (!userId) return;
            setLoading(true);
            const res = await axios.post(`${BASE_URL}/order/makeOrder`, {user_id: userId});
            toast.success(res.data.msg || 'Buyurtma berildi');
            const productCount = await fetchBasket();
            if (productCount === 0) await fetchOrders();
            setShowInput(false);
        } catch {
                        ...
                        {items.map(item => (
                            <div key={item._id}
                                ...
                                <div className="w-[90px] h-[100px] flex items-center justify-center overflow-hidden">
                                    <Image src={`${BASE_IMAGE_URL}/${item.product.image}`}
